...
# Helper functions
prettify_data_col <- . %>%
    mutate_at(vars(matches("data")), funs(str_trunc(., width = 50, ellipsis = "\t...")))
    ...
    map(function(failing_table) {
        ...
        tagList(
            tags$h3(
                ...
                failing_table$Title,
                htmltools::tags$a(href=ref_link, target = "_blank", '(see full description)')
            ),
            failing_table$results[[1]] %>% ungroup %>% prettify_data_col() %>% datatable()
        )
